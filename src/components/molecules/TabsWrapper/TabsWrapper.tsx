...
const TabsWrapper: FC<ITabsWrapper> = ({ tabsNames, tabsContains }) => {
  const [tabIndex, setTabIndex] = useState(0);

  const tabsHeader = useMemo(
    () =>
      tabsNames.map((tabName, i) => (
        <Tab key={tabName} label={tabName} id={i.toString()} sx={tabStyle} />
      )),
    [tabsNames]
  );
  ...
  const tabsPanels = useMemo(
    () =>
      tabsContains.map((contain, i) => (
        <Box key={i} hidden={tabIndex !== i} className={style.tabContain}>
          {contain}
        </Box>
      )),
